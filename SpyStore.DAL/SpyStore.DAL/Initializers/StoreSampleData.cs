...
namespace SpyStore.DAL.Initializers
{
    public static class StoreSampleData
    {
        ...
        public static List<Order> GetOrders(Customer customer, StoreContext context)
        {
            ...
            order.OrderDetails = GetOrderDetails(order, context);
        ...
        public static List<OrderDetail> GetOrderDetails(Order order, StoreContext context)
        {
            var firstProduct = context.Products.FirstOrDefault();
            var prod1 = context.Categories.Include(c => c.Products).FirstOrDefault()?.Products.FirstOrDefault() ?? firstProduct;
            var prod2 = context.Categories.Skip(1).Include(c => c.Products).FirstOrDefault()?.Products.FirstOrDefault() ?? firstProduct;
            var prod3 = context.Categories.Skip(2).Include(c => c.Products).FirstOrDefault()?.Products.FirstOrDefault() ?? firstProduct;
